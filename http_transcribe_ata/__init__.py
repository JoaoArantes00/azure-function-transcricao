...
def _blob_service_client():
    # Usa MSI (DefaultAzureCredential) para acessar a conta informada
...
def _speech_transcribe_from_bytes(audio_bytes: bytes, language: str = "pt-BR") -> dict:
    """
    Transcrever áudio usando Azure Speech com timestamps.
    Estratégia: salvar WAV em arquivo temporário e usar AudioConfig(filename=...),
    deixando o SDK ler o cabeçalho (formato/codec).
    """
    ...
    try:
        ...
        speech_config.set_property(speechsdk.PropertyId.SpeechServiceResponse_RequestDetailedResultTrue, "true")
        speech_config.set_property(speechsdk.PropertyId.SpeechServiceConnection_EndSilenceTimeoutMs, "5000")
        speech_config.set_property(speechsdk.PropertyId.SpeechServiceConnection_InitialSilenceTimeoutMs, os.getenv("SPEECH_INITIAL_SILENCE_MS", "10000"))

        # Salva em arquivo temporário (o SDK lida com o WAV corretamente)
        ...
        def recognized_handler(evt):
            ...
            if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech and evt.result.text:
                start_time = evt.result.offset / 10_000_000  # 100-ns to seconds
        ...
        # Timeout proporcional à duração do arquivo (min 30s, máx 180s)
